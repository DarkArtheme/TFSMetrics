...
func newMetrics(auther string, project string) *metrics {
	m := &metrics{
		commits: prometheus.NewCounter(prometheus.CounterOpts{
			Name:        "commits",
			Help:        "commits counter",
			ConstLabels: map[string]string{"auther": auther, "project": project},
		}),
...
func (e *exporter) GetProjectMetrics(iterator repointerface.CommitIterator, project string) {
	for commit, err := iterator.Next(); err == nil; commit, err = iterator.Next() {
		if m, ok := e.authers[commit.Author]; ok {
			...
			e.authers[commit.Author] = newMetrics(commit.Author, project)
